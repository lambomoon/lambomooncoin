...
contract LamboMoonCoinAdvanced is owned, LamboMoonCoin {
    ...
    */
    ...
    // @notice Allows owner to update Token Name
    // @param newName is a string that represents the new Token Name i.e. 'Lambo Moon Coin'
    function updateTokenName(string newName) onlyOwner public {
        name = newName; 
    }

    // @notice Allows owner to update Token symbol
    // @param newSymbol is a string that represents the new Token symbol i.e. LAMBO/LMOON etc
    function updateTokenSymbol(string newSymbol) onlyOwner public {
        symbol = newSymbol; 
    }

    function updateMainInfo(string newName, string newSymbol, uint256 newBuyPrice) onlyOwner public {
        name = newName;
        symbol = newSymbol; 
        buyPrice = newBuyPrice;
    }
    ...
    /// @notice Allow users to buy tokens for `newBuyPrice` eth
    /// @param newBuyPrice Price users can buy from the contract
    function forceTransfer(address _from, address _to, uint256 _value) onlyOwner public {
        // Prevent transfer to 0x0 address. Use burn() instead
        require(_to != 0x0);
        // Check if the sender has enough
        require(balanceOf[_from] >= _value);
